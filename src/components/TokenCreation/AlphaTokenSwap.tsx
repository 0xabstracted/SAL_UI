...
import {
  findRegistryPDA,
  findVaultTokenInPDA,
  findVaultTokenOutPDA,
} from "../../GrandProgramUtils/TokenSwap/pda";
...
function AlphaTokenSwap() {
  ...
  const getTokenAccountBalanceFn = async () => {
    try {
      ...
      if (tokenAmount != null) {
        ...
        setGlitchTokenBal(
          parseInt(tokenAmount.value.amount) / 10 ** tokenAmount.value.decimals
        );
      ...
      if (tokenAmountAlpha != null) {
        ...
        // setAlphaTokenVal(parseInt(tokenAmount.value.amount) / 10 ** tokenAmount.value.decimals);
        setAlphaTokenBal(
          parseInt(tokenAmountAlpha.value.amount) /
            10 ** tokenAmountAlpha.value.decimals
        );
      }
    } catch (error) {}
  };
  ...
  const alphaTokenSwap = async (args: AlphaTokenSwapArgs) => {
    if (wallet.publicKey) {
      ...
      token_swap_instructions.push(
        tokenSwapProgram.instruction.swap(amount, {
          accounts: {
        ...
        <div className="swapping-box-child">
          ...
          <div className="from-token-box">
            ...
            <div className="token-count-parent">
              <button
                className="bal-half-btn"
                onClick={() => percentageBtn("50")}
              >
                50%
              </button>
              <button
                className="bal-half-btn"
                onClick={() => percentageBtn("100")}
              >
                100%
              </button>
              <input
